...
def train(train_env, action_space= 5, num_episodes = 1000, q_path = None):
    ...
    for episode in range(start_episode, num_episodes):
        ...
        plt.savefig("C:\\Users\\alakh\\OneDrive\\Desktop\\Multi_Agent_RL-main\\plot.png")
        if (episode+1)%10 == 0:
            with open(f"C:\\Users\\alakh\\OneDrive\\Desktop\\Multi_Agent_RL-main\\QTable_{episode+1}.pkl", "wb") as fi:
                pkl.dump(Q_table, fi)
...
def update_qvalue(Q_table, action_space, obs, a_list, r, obs_prime, no_of_agents):
    for i in range(no_of_agents):
        ...
        if obs[f"A_{i}"][1] == -1:
            ...
            Q_table[0][s][a] = Q_table[0][s][a] + learning_rate * (r + np.max(q_next) - Q_table[0][s][a])
        else:
            ...
            Q_table[1][s][a] = Q_table[1][s][a] + learning_rate * (r + np.max(q_next) - Q_table[1][s][a])
